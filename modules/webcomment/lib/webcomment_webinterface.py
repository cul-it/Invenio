    ...
    def display(self, req, form):
        ...
        if ok:
            ...
            tabs = [(unordered_tabs[tab_id]['label'], \
                     '%s/%s/%s/%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, tab_id, link_ln), \
            ...
            if navtrail:
                navtrail += ' &gt; '
            navtrail += '<a class="navtrail" href="%s/%s/%s?ln=%s">'% (CFG_SITE_URL, CFG_RECORD_URI, self.recid, argd['ln'])
            navtrail += title
            navtrail += '</a>'
...
##                             fp.write(formfield.file.read(10240))
        ...
        if ok:
            ...
            if not argd['msg']:
                ...
                try:
                    ...
                    return redirect_to_url(req, CFG_SITE_URL + '/'+ CFG_RECORD_URI +'/' + \
    ...
    def vote(self, req, form):
        ...
        if argd['referer']:
            ...
            referer %= (CFG_SITE_URL, CFG_RECORD_URI, self.recid, self.discussion == 1 and 'reviews' or 'comments', argd['ln'])
    ...
    def subscribe(self, req, form):
        ...
        display_url = "%s/%s/%s/comments/display?subscribed=%s&ln=%s" % \
                      (CFG_SITE_URL, CFG_RECORD_URI, self.recid, str(success), argd['ln'])
...
class WebInterfaceCommentsFiles(WebInterfaceDirectory):
    ...
    def _lookup(self, component, path):
        ...
        CFG_SITE_URL/CFG_RECORD_URI/5953/comments/attachments/get/652/myfile.pdf
        """
...
##         user_files_path = '%(CFG_SITE_URL)s/%(CFG_RECORD_URI)s/%(recid)i/comments/attachments/get/%(uid)s' % \
##                           {'uid': uid,
##                            'recid': self.recid,
##                            'CFG_SITE_URL': CFG_SITE_URL}
