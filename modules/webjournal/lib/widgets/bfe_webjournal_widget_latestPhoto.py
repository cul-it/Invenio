...
from invenio.config import CFG_CERN_SITE, CFG_SITE_URL, CFG_RECORD_URI
...
def get_widget_html(language, max_photos, collections, separator, ln):
    ...
    # Build output
    html_out = separator.join(['<a href="%s/%s/%i?ln=%s"><img class="phr" width="100" height="67" src="%s"/>%s</a>' % (CFG_SITE_URL, CFG_RECORD_URI, recid, ln, photo_url, title) for (recid, photo_url, title) in images_urls])
