    ...
    def import_descriptions_and_comments_from_marc(self, record=None):
        ...
        for field in fields:
            ...
            if url:
                ## Given a url
                url = url[0]
                if url == '%s/%s/%s/files/' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid):
                    ## If it is a traditional /CFG_RECORD_URI/1/files/ one
...
class BibDocFile:
    ...
    def __init__(self, fullpath, doctype, version, name, format, recid, docid, status, checksum, more_info, human_readable=False):
        ...
        if self.subformat:
            self.url = create_url('%s/%s/%s/files/%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, self.name, self.superformat), {'subformat' : self.subformat})
            self.fullurl = create_url('%s/%s/%s/files/%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, self.name, self.superformat), {'subformat' : self.subformat, 'version' : self.version})
        else:
            self.url = create_url('%s/%s/%s/files/%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, self.name, self.superformat), {})
            self.fullurl = create_url('%s/%s/%s/files/%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, self.name, self.superformat), {'version' : self.version})
...
def bibdocfile_url_to_bibrecdocs(url):
    """Given an URL in the form CFG_SITE_[SECURE_]URL/CFG_RECORD_URI/xxx/files/... it returns
...
def bibdocfile_url_p(url):
    ...
    if url.startswith('%s/getfile.py' % CFG_SITE_URL) or url.startswith('%s/getfile.py' % CFG_SITE_SECURE_URL):
        return True
    if not (url.startswith('%s/%s/' % (CFG_SITE_URL, CFG_RECORD_URI)) or url.startswith('%s/%s/' % (CFG_SITE_SECURE_URL, CFG_RECORD_URI))):
        return False
...
def decompose_bibdocfile_url(url):
    ...
    if url.startswith('%s/%s/' % (CFG_SITE_URL, CFG_RECORD_URI)):
        recid_file = url[len('%s/%s/' % (CFG_SITE_URL, CFG_RECORD_URI)):]
    elif url.startswith('%s/%s/' % (CFG_SITE_SECURE_URL, CFG_RECORD_URI)):
        recid_file = url[len('%s/%s/' % (CFG_SITE_SECURE_URL, CFG_RECORD_URI)):]
    else:
    ...
    if not recid and docname.isdigit():
        ## If the URL was something similar to CFG_SITE_URL/CFG_RECORD_URI/123
        return (int(docname), '', '')
    return (int(recid), docname, format)

re_bibdocfile_old_url = re.compile(r'/%s/(\d*)/files/' % CFG_RECORD_URI)
def decompose_bibdocfile_old_url(url):
    """Given a bibdocfile old url (e.g. CFG_SITE_URL/CFG_RECORD_URI/123/files)
