...
class WebInterfaceFilesPages(WebInterfaceDirectory):
    ...
    def _lookup(self, component, path):
        # after /<CFG_RECORD_URI>/<recid>/files/ every part is used as the file
        # name
        filename = component
        ...
        def getfile(req, form):
            ...
            if uid == -1 or CFG_ACCESS_CONTROL_LEVEL_SITE > 1:
                return page_not_authorized(req, "/%s/%s" % (CFG_RECORD_URI, self.recid),
                                           navmenuid='submit')
            ...
            return pageheaderonly(title=title,
                        navtrail=create_navtrail_links(cc=cc, aas=0, ln=ln) + \
                                        ...
                                        &gt; %s''' % \
                        (CFG_SITE_URL, CFG_RECORD_URI, self.recid, title, _("Access to Fulltext")),
    ...
    def __call__(self, req, form):
        """Called in case of URLs like /CFG_RECORD_URI/123/files without
           trailing slash.
        """
        args = wash_urlargd(form, websubmit_templates.files_default_urlargd)
        ln = args['ln']
        ...
        return redirect_to_url(req, '%s/%s/%s/files/%s' % (CFG_SITE_URL, CFG_RECORD_URI, self.recid, link_ln))
...
def websubmit_legacy_getfile(req, form):
    ...
    def _getfile_py(req, recid=0, docid=0, version="", name="", format="", ln=CFG_SITE_LANG):
        ...
        redirect_to_url(req, '%s/%s/%s/files/%s%s?ln=%s%s' % (CFG_SITE_URL, CFG_RECORD_URI, recid, name, format, ln, version and 'version=%s' % version or ''), apache.HTTP_MOVED_PERMANENTLY)
