    ...
    def __call__(self, req, form):
        ...
        if argd['id'] and argd['format']:
            ...
            if format in formats:
                redirect_to_url(req, '%s/%s/%s/export/%s' % (CFG_SITE_URL, CFG_RECORD_URI, argd['id'], format))
            else:
...
class WebInterfaceRecordPages(WebInterfaceDirectory):
    """ Handling of a /CFG_RECORD_URI/<recid> URL fragment """
    ...
    # Return the same page wether we ask for /CFG_RECORD_URI/123 or /CFG_RECORD_URI/123/
    index = __call__
...
class WebInterfaceLegacySearchPages(WebInterfaceDirectory):
    ...
    def __call__(self, req, form):
        ...
        # /CFG_RECORD_URI/... display if a recid is requested
        if argd['recid'] != -1:
            target = '/%s/%d' % (CFG_RECORD_URI,argd['recid'])
            del argd['recid']
...
class WebInterfaceSearchInterfacePages(WebInterfaceDirectory):
    ...
    def _lookup(self, component, path):
        ...
        elif component == CFG_RECORD_URI and path and path[0] == 'merge':
            return WebInterfaceMergePages(), path[1:]

        elif component == CFG_RECORD_URI and path and path[0] == 'edit':
            return WebInterfaceEditPages(), path[1:]

        elif component == CFG_RECORD_URI and path and path[0] == 'multiedit':
            return WebInterfaceMultiEditPages(), path[1:]

        elif component == CFG_RECORD_URI or component == 'record-restricted':
            try:
                if CFG_WEBSEARCH_USE_ALEPH_SYSNOS:
                    # let us try to recognize /<CFG_RECORD_URI>/<SYSNO> style of URLs:
                ...
                # display record #1 for URL /CFG_RECORD_URI without a number
                recid = 1
            except ValueError:
                if path[0] == '':
                    # display record #1 for URL /CFG_RECORD_URI/ without a number
                    ...
                    # display page not found for URLs like /CFG_RECORD_URI/foo
                    return None, []

            if recid <= 0:
                # display page not found for URLs like /CFG_RECORD_URI/-5 or /CFG_RECORD_URI/0
                return None, []
...
#                    format = path[1]
                    ...
                    # display page not found for URLs like /CFG_RECORD_URI/references
...
class WebInterfaceRecordExport(WebInterfaceDirectory):
    """ Handling of a /<CFG_RECORD_URI>/<recid>/export/<format> URL fragment """
    ...
    # Return the same page wether we ask for /CFG_RECORD_URI/123/export/xm or /CFG_RECORD_URI/123/export/xm/
    index = __call__
