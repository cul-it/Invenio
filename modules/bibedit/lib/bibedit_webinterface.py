...
from invenio.config import CFG_SITE_LANG, CFG_SITE_URL, CFG_RECORD_URI
...
class WebInterfaceEditPages(WebInterfaceDirectory):
        ...
        * Calling the appropriate function from the engine.
        ...
        if user_info['email'] == 'guest':
            ...
            if not ajax_request:
                ...
                referer = '/edit/'
                if self.recid:
                    referer = '/%s/%s/edit/' % (CFG_RECORD_URI, self.recid)
            ...
            redirect_to_url(req, '%s/%s/edit/#state=edit&recid=%s&recrev=%s' % (
                    CFG_SITE_URL, CFG_RECORD_URI, self.recid, ""))
    ...
    def __call__(self, req, form):
        ...
        if self.recid:
            ...
            redirect_to_url(req, '%s/%s/edit/' % CFG_SITE_URL, CFG_RECORD_URI)
