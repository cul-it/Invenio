...
from invenio.config import CFG_SITE_URL, CFG_SITE_LANG, CFG_RECORD_URI
...
class BibFormatAPITest(unittest.TestCase):
    ...
    def test_basic_formatting(self):
        ...
        pageurl = CFG_SITE_URL + '/%s/73?of=hx' % CFG_RECORD_URI
...
@article{Wang:74,
    ...
    def test_bibtex_output(self):
        ...
        pageurl = CFG_SITE_URL + '/%s/74?of=hx' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
class BibFormatDetailedHTMLTest(unittest.TestCase):
 ...
 Légende</span><br /><small>Conference "Internet, Web, What's next?" le 26 juin 1998 au CERN: Tim Berners-Lee, inventeur du World-Wide Web et directeur du W3C, explique comment le Web est ne, et donne ses opinions sur l'avenir.</small></p>'''
        self.record_7_hd_resource = '''<img src="%s/%s/7/files/9806033.gif?subformat=icon" alt="9806033" style="max-width:250px;_width:250px;" />''' % (CFG_SITE_URL, CFG_RECORD_URI)
        self.record_7_hd_resource_link = '%s/%s/7/files/9806033.jpeg' %  (CFG_SITE_URL, CFG_RECORD_URI)

    def test_detailed_html_output(self):
        """bibformat - Detailed HTML output"""

        # Test record 74 (Article)
        pageurl = CFG_SITE_URL + '/%s/74?of=hd' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
        ...
        pageurl = CFG_SITE_URL + '/%s/7?of=hd' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
<article xmlns:xlink="http://www.w3.org/1999/xlink/">
  <front>
    ...
    <article-meta>
      ...
      <fpage/>
      <lpage/>
      <self-uri xlink:href="%(siteurl)s/%(CFG_RECORD_URI)s/70"/>
      <self-uri xlink:href="%(siteurl)s/%(CFG_RECORD_URI)s/70/files/0003291.pdf"/>
      <self-uri xlink:href="%(siteurl)s/%(CFG_RECORD_URI)s/70/files/0003291.ps.gz"/>
    </article-meta>
...
</articles>''' % {'siteurl': CFG_SITE_URL, 'CFG_RECORD_URI': CFG_RECORD_URI}

    def test_nlm_output(self):
        """bibformat - NLM output"""

        pageurl = CFG_SITE_URL + '/%s/70?of=xn' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
γεμάτος περιπέτειες, γεμάτος γνώσεις [...] </small>''' % (CFG_SITE_URL, CFG_SITE_LANG)
    ...
    def test_brief_html_output(self):
        ...
        pageurl = CFG_SITE_URL + '/%s/76?of=HB' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
<record>
  ...
  <datafield tag="980" ind1=" " ind2=" ">
    ...
    def test_marcxml_output(self):
        ...
        pageurl = CFG_SITE_URL + '/%s/9?of=xm' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
000000029 980__ $$aBOOK'''
    ...
    def test_marc_output(self):
        ...
        pageurl = CFG_SITE_URL + '/%s/29?of=hm' % CFG_RECORD_URI
        result = test_web_page_content(pageurl,
...
class BibFormatPublInfoFormattingTest(unittest.TestCase):
    ...
    def test_publinfo_in_html_detailed(self):
        ...
        self.assertEqual([],
          test_web_page_content(CFG_SITE_URL + '/%s/84' % CFG_RECORD_URI,
            expected_text="Nucl. Phys. B: 656 (2003) pp. 23-36"))
