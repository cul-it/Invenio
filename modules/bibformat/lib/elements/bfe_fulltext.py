...
from invenio.config import CFG_SITE_URL, CFG_CERN_SITE, CFG_RECORD_URI
...
def format_element(bfo, style, separator='; ', show_icons='no', focus_on_main_file='no', show_subformat_icons='no'):
    ...
    additional_str = ''
    if additionals:
        additional_str = ' <small>(<a '+style+' href="'+CFG_SITE_URL+'/%s/'+str(bfo.recID)+'/files/">%s</a>)</small>' % (CFG_RECORD_URI, _("additional files"))

    versions_str = ''
    #if old_versions:
        #versions_str = ' <small>(<a '+style+' href="'+CFG_SITE_URL+'/CFG_RECORD_URI/'+str(bfo.recID)+'/files/">%s</a>)</small>' % _("older versions")
...
def get_files(bfo, distinguish_main_and_additional_files=True, include_subformat_icons=False):
    ...
    {'main_urls' : {'Main'      : [('http://CFG_SITE_URL/CFG_RECORD_URI/1/files/aFile.pdf', 'aFile', 'PDF'),
                                   ('http://CFG_SITE_URL/CFG_RECORD_URI/1/files/aFile.gif', 'aFile', 'GIF')],
                    'Additional': [('http://CFG_SITE_URL/CFG_RECORD_URI/1/files/bFile.pdf', 'bFile', 'PDF')]},
